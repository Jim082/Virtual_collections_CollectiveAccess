 	...
 	class DetailController extends FindController {
 		...
 		public function __call($ps_function, $pa_args) {
			...
			#
 			...
 			$htmlPrev = caGetOption('previousLink', $va_options, _t('Previous'));
			#$htmlPrev = <i class='fa fa-angle-left'></i><div class='small'>Prev</div>
			$htmlPrev = substr_replace ( $htmlPrev , _t('Prev') , 51,4 );
 			$this->view->setVar('previousLink', ($vn_previous_id > 0) ? caDetailLink($this->request, $htmlPrev, '', $vs_table, $vn_previous_id) : '');
			
			$htmlNext = caGetOption('nextLink', $va_options, _t('Next'));
			#$htmlNext = <i class='fa fa-angle-right'></i><div class='small'>Next</div>
			$htmlNext = substr_replace ( $htmlNext , _t('Next') , 52,4 );
 			$this->view->setVar('nextLink', ($vn_next_id > 0) ? caDetailLink($this->request, $htmlNext, '', $vs_table, $vn_next_id) : '');
			
 			$va_params = array();
 			$va_params["row_id"] = $t_subject->getPrimaryKey(); # --- used to jump to the last viewed item in the search/browse results
			
			$htmlBack = caGetOption('resultsLink', $va_options, _t('Back'));
			#$htmlBack = <i class='fa fa-angle-double-left'></i><div class='small'>Back</div>
			$htmlBack = substr_replace ( $htmlBack , _t('Back') , 58,4 );
 			$this->view->setVar('resultsLink', ResultContext::getResultsLinkForLastFind($this->request, $vs_table, $htmlBack, null, $va_params));
 			...
 			if (method_exists($t_subject, 'getPrimaryRepresentationInstance')) {
				...
				#print_r ("---".$t_representation->getMediaInfo('media', 'original', 'MIMETYPE')); --> ---audio/mpeg
					...
					caRepresentationViewer(
						...
						array_merge($va_options, $va_media_display_info, 
							array(
								...
								'dontShowPlaceholder' => caGetOption('representationViewerDontShowPlaceholder', $va_options, true), 
								'captionTemplate' => caGetOption('representationViewerCaptionTemplate', $va_options, false)
							)
						)
					)
				);
				#print_r (caGetMediaDisplayInfo('detail', $t_representation->getMediaInfo('media', 'original', 'MIMETYPE')));
				#print_r ($t_representation->getMediaInfo('media', 'original', 'MIMETYPE'));
			}
