	...
	function caGetMediaViewerHTML($po_request, $ps_identifier, $pt_subject, $pa_options=null) {
		...
		switch($va_identifier['type']) {
			case 'representation':
				...
				if ($pb_inline) {
					if ($t_instance->get('ca_object_representations.media_view') != "" and strpos($t_instance->get('ca_object_representations.media_info'),"View:") == 0) {
 	...
 	function caRepresentationViewerHTMLBundles($po_request, $po_data, $pt_subject, $pa_options=null) {
 		...
 		while($po_data->nextHit()) {
 			...
 			foreach($va_representation_ids as $vn_representation_id) {
				...
				if ($po_data->get('ca_object_representations.media_view') != "" and strpos($po_data->get('ca_object_representations.media_info'),"View:") == 0) {
