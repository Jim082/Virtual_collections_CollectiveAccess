		...
		switch($va_identifier['type']) {
			case 'representation':
				...
				if ($pb_inline) {
					...
					if ($t_instance->getWithTemplate("^ca_object_representations.media_info") != ""){
						if ($t_instance->getWithTemplate("^ca_object_representations.media_view") != ""){
							$vs_caption = $t_instance->getWithTemplate("<div class='small'>^ca_object_representations.media_info , ^ca_object_representations.media_view view</div>");
						}else{
							$vs_caption = $t_instance->getWithTemplate("<div class='small'>^ca_object_representations.media_info</div>");
						}
					}else{
						$vs_caption = $t_instance->getWithTemplate("<div class='small'>^ca_object_representations.preferred_labels.name</div>");
					}
 	...
 	function caRepresentationViewerHTMLBundles($po_request, $po_data, $pt_subject, $pa_options=null) {
 		...
 		while($po_data->nextHit()) {
 			...
 			foreach($va_representation_ids as $vn_representation_id) {
				...
				if ($po_data->getWithTemplate("^ca_object_representations.media_info") != ""){
					if ($po_data->getWithTemplate("^ca_object_representations.media_view") != ""){
						$vs_caption = $po_data->getWithTemplate("<div class='small'>^ca_object_representations.media_info , ^ca_object_representations.media_view view</div>");
					}else{
						$vs_caption = $po_data->getWithTemplate("<div class='small'>^ca_object_representations.media_info</div>");
					}
				}else{
					$vs_caption = $po_data->getWithTemplate("<div class='small'>^ca_object_representations.preferred_labels.name</div>");
				}
