 ...
 */
	...
	#recalculate number of hits to remove type "document"
	$docfound = 0;
	...
	if (intval($docfound) == 0){
		$vn_result_size 	= (sizeof($va_criteria) > 0) ? $qr_res->numHits() : $this->getVar('totalRecordsAvailable');
	}else{
		$vn_result_size 	= (sizeof($va_criteria) > 0) ? $qr_res->numHits() - intval($docfound): $this->getVar('totalRecordsAvailable') - intval($docfound);
	}
...
<?php
					...
					if (sizeof($va_criteria) > 0) {
						foreach($va_criteria as $va_criterion) {
							...
							if (strpos($va_criterion['value'], 'Anthropology') !== false) {
								$valdoctype = "anthropo";
							}
							if (strpos($va_criterion['value'], 'Zoology') !== false) {
								$valdoctype = "zoology";
							}
						}
					}
					...
					if(is_array($va_export_formats) && sizeof($va_export_formats)){
						...
						foreach($va_export_formats as $va_export_format){
							#print $va_export_format["code"]."     ".$valdoctype."--";
							if (strpos($va_export_format["code"], 'excel') !== false) {
									switch ($valdoctype) {
										case "zoology":
											if (strpos($va_export_format["code"], 'zoo') !== false) {
											...
											break;
										case "anthropo":
											if (strpos($va_export_format["code"], 'anthropo') !== false) {
												print "<li class='".$va_export_format["code"]."'>".caNavLink($this->request, $va_export_format["name"], "", "*", "*", "*", array("view" => "pdf", "download" => true, "export_format" => $va_export_format["code"], "key" => $vs_browse_key))."</li>";
											}
											break;
										default:
											if (strpos($va_export_format["code"], 'all') !== false) {
												print "<li class='".$va_export_format["code"]."'>".caNavLink($this->request, $va_export_format["name"], "", "*", "*", "*", array("view" => "pdf", "download" => true, "export_format" => $va_export_format["code"], "key" => $vs_browse_key))."</li>";
											}
											break;
									}
								...
								if (strpos($va_export_format["code"], 'ppt') === false) {
									print "<li class='".$va_export_format["code"]."'>".caNavLink($this->request, $va_export_format["name"], "", "*", "*", "*", array("view" => "pdf", "download" => true, "export_format" => $va_export_format["code"], "key" => $vs_browse_key))."</li>";
								}
							}
						}
		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {				
				...
				if ($va_criterion['facet_name'] != '_search') {
					...
					$val = str_replace("???",_t("Theme"),$va_criterion['value']);
					print ' '.$val;
...
if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_results')){
	...
	#$vs_result_page = $this->render("Browse/browse_results_{$vs_current_view}_html.php", ['valdoctype'=>$valdoctype]);
			...
			if(is_array($va_views) && (sizeof($va_views) > 1)){
				foreach($va_views as $vs_view => $va_view_info) {
					if ($vs_current_view === $vs_view) {
						if ($vs_view === 'table') {
							print '<a href="#" class="active"><img src="/providence/pawtucket/themes/VirtualCollections/assets/pawtucket/graphics/table.jpg" style="vertical-align:top" alt="Table" /></a> ';
						} else {
						...
						if ($vs_view === 'table') {
							print caNavLink($this->request, '<img src="/providence/pawtucket/themes/VirtualCollections/assets/pawtucket/graphics/table.jpg" style="vertical-align:top" alt="Table" />&nbsp;', 'disabled', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key));
						} else {
