 ...
 */
	...
	# retrieve results from nbr of objects in facet collection-----------------
	$stages=Array();
	...
	$sql = "SELECT * FROM `total_numbers_in_upper_collections`;";

	$q = $conn->prepare($sql);
	$q->execute();
	$Collamounts = $q->fetchAll();
	$i=0; 
	foreach($Collamounts as $Collamount){
		$Colamounts[$i]['coll']= $Collamount[0];
		$Colamounts[$i]['amount']= $Collamount[1];
		$i++;
	}
	#end retrieve results from nbr of objects in facet collection-----------------
...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			foreach($Colamounts as $Colamount){
				if ($Colamount['coll'] == $va_item['label']){
					$amountcol = " (".$Colamount['amount'].")";
				}
			}

		    $vs_content_count = (isset($va_item['content_count']) && ($va_item['content_count'] > 0)) ? " (".$va_item['content_count'].")" : $amountcol;
