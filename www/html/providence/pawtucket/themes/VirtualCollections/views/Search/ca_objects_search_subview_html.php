...
<?php
		...
		while($qr_results->nextHit()) {
			#if ( $qr_results->get('ca_objects.type_id') != 23) { #show Viewer only if it's not a document (txt, csv) 
?>	
				<div class='{{{block}}}Result multisearchResult'>
<?php 				$vs_image = $qr_results->get('ca_object_representations.media.widepreview', array("checkAccess" => $va_access_values));
					if(!$vs_image){
